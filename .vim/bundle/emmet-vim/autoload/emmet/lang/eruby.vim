...
function! emmet#lang#eruby#findTokens(str) abort
  let str = a:str
...
function! emmet#lang#eruby#parseIntoTree(abbr, type) abort
  let abbr = a:abbr
  let type = a:type
...
function! emmet#lang#eruby#toString(settings, current, type, inline, filters, itemno, indent) abort
...
function! emmet#lang#eruby#imageSize() abort
  ...
  let current = emmet#lang#eruby#parseTag(content)
...
function! emmet#lang#eruby#encodeImage() abort
  let img_region = emmet#util#searchRegion('<img\s', '>')
  if !emmet#util#regionIsValid(img_region) || !emmet#util#cursorInRegion(img_region)
    return
  endif
...
function! emmet#lang#eruby#parseTag(tag) abort
  let current = emmet#newNode()
...
function! emmet#lang#eruby#toggleComment() abort
...
function! emmet#lang#eruby#balanceTag(flag) range abort
...
function! emmet#lang#eruby#moveNextPrevItem(flag) abort
...
function! emmet#lang#eruby#moveNextPrev(flag) abort
...
function! emmet#lang#eruby#splitJoinTag() abort
  let curpos = emmet#util#getcurpos()
  while 1
...
function! emmet#lang#eruby#removeTag() abort
  let curpos = emmet#util#getcurpos()
  while 1
    let mx = '<\(/\{0,1}[a-zA-Z][a-zA-Z0-9:_\-]*\)[^>]*'
    let pos1 = searchpos(mx, 'bcnW')
